...
volatile  float th_new[6];
volatile  float th[6];
volatile  float th_old[6];*/
volatile  float th[6];
...
//  delay(1000);
 ...
 /* pinMode(stepPin_1,OUTPUT); 
  ...
  pinMode(dirPin_6,OUTPUT);*/
        ...
        lcd.setCursor(0, 0);  lcd.print(th[1]);
        lcd.setCursor(6, 0);  lcd.print(th[2]);
        lcd.setCursor(12, 0); lcd.print(th[3]);
        lcd.setCursor(0, 1);  lcd.print(th[4]);
        lcd.setCursor(6, 1);  lcd.print(th[5]);
...
/*if(Serial.available() >= 5) {
    ...
    for(int i = 1; i < 7; i ++) {
        ...
        th_new[i] = Serial.parseFloat();
        th[i] = th_new[i] - th_old[i];
        th_old[i] = th_new[i];
